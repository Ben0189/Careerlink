        ...
        public async Task<List<PostDTO>> SearchPost(int experienceLevel, List<int> skillIds)
        {
            ...
            var postDtos = filteredPosts.Select(post => new PostDTO
            {
                ...
                User = new UserDTO
                {
                    ...
                    PhoneNumber = post.User.PhoneNumber ?? "Unknown",
        ...
        public async Task<PostDTO> CreatePost(PostDTO postDto)
        {
            ...
            return new PostDTO
            {
                ...
                User = new UserDTO
                {
                    ...
                    UserName = newPost.User.UserName ?? "Unknown",
                    Email = newPost.User.Email ?? "Unknown",
                    PhoneNumber = newPost.User.PhoneNumber ?? "Unknown",
                },
                Description = newPost.Description,
                experienceLevel = newPost.experienceLevel,
                CreatedDate = newPost.CreatedDate,
                UpdatedDate = newPost.UpdatedDate,
                SkillIds = newPost.Skills.Select(s => s.SkillId).ToList()
            };
        }

        public async Task<string> UpdatePost(int postId, PostDTO postDto)
        {
            var existingPost = await _dbContext.Posts.Include(p => p.Skills).FirstOrDefaultAsync(p => p.PostId == postId);
            ...
            if (postDto.SkillIds != null && postDto.SkillIds.Count > 0)
            {
                existingPost.Skills = await _dbContext.Skills.Where(skill => postDto.SkillIds.Contains(skill.SkillId)).ToListAsync();
            }
            await _dbContext.SaveChangesAsync();
